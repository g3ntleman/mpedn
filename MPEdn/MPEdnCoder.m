...
- (void) writeObject: (id <MPEdnCoding>) object {
    ...
    if (! [object conformsToProtocol: @protocol(MPEdnCoding)]) {
...
+ (void) registerDefaultClass: (Class) aClass {
    NSParameterAssert([aClass conformsToProtocol: @protocol(MPEdnTaggedCoding)]);
...
- (NSString*) objectToEdnString {
    return [[[MPEdnCoder alloc] init] ednFromObject: (id<MPEdnCoding>)self];
}
...
@implementation NSString (MPEdnCoder)

- (id) ednStringToObject {
    return [[[MPEdnCoder alloc] init] parseString: self];
}
