...
/**
 * Codes for parse errors reported in EdnCoder.error.
 */
typedef enum {
    ERROR_OK,
    ERROR_INVALID_TOKEN,
 ...
 *
 * See [NSString(MPEdnCoder) ednStringToObject]
 * and [NSString(MPEdnCoder) ednStringToObjectKeywordsAsStrings]
 */
...
+ (void) registerDefaultClass: (Class <MPEdnTaggedCoding>) aClass;

- (Class) registeredClassForEdnTag: (NSString*) tag;
- (void) deregisterClass: (Class <MPEdnTaggedCoding>) aClass;
- (void) registerClass: (Class <MPEdnTaggedCoding>) aClass;
 ...
 */
- (NSString *) ednFromObject: (id <MPEdnCoding>) value;
...
@interface NSString (MPEdnCoder)

/**
 * Shortcut to parse a single string with [MPEdnCoder parseString:].
 */
